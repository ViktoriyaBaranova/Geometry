...
const createTask=()=>{
    ...
    condition.innerHTML = `<h2>Задание 1.2. Векторы. Основные понятия</h2><p>Начиная от конца вектора ${nameVect[nameVect.length-1]}, последовательно отложите векторы, удовлетворяющие следующим условиям:</p>`;
...
btnCheck.addEventListener("click", () => {
    ...
    data[variant].userVectors.forEach((el, ind)=>{
        count += (el.every((val, ind)=>val == data[variant].taskVectors[i][ind])) ? 1 : -1;
        i++;
    });
