...
const sizeSvg = 19, numberOfClickCircles = 2, quantityGreyVect = 15;
...
const createNameVect = (arr, name, ind)=>{
    ...
    txt.setAttribute('y', arr[3]*10+5);
    txt.textContent = name[ind];
    svg.appendChild(txt);
}
const createTask=()=>{
    variant = randomInt(0, data.length);
    condition.innerHTML = `<h2>Задание 2.2. Линейные операции над векторами</h2><p>Последовательно отложите следующие векторы, начиная от конца вектора c:</p>`;
    data[variant].basis.map((el, ind)=>{
        const coef = randomInt2(0, 2);
        ind === 0? el[2]+=coef : el[3]-=coef;
    });
    data[variant].lines.forEach(el=>draw_svg_line(el[0], el[1], el[2], el[3]));
    data[variant].basis.forEach(el=>draw_svg_vect(el[0], el[1], el[2], el[3]));
    const [startVector, dataVectors] = generateDataVect(variant);
    
    const divs = document.querySelectorAll('.div-task');
    const c = [];
    const coordsBasisVectors = coordVect(data[variant].basis);
    const coordTaskVect = coordVect(dataVectors);
    data[variant].coordTaskVectors = coordTaskVect;
    coordTaskVect.forEach((el, ind)=>{
        c.push([el[0]/coordsBasisVectors[0][0], el[1]/coordsBasisVectors[1][1]]);
    });
    dataVectors.forEach((_, index)=>{
        const txtTask = document.createElement("p");
        txtTask.style.opacity = 0.5;
        txtTask.classList.add("task");
        txtTask.textContent = `c${index+1} = ${c[index][0]} a + ${c[index][1]} b;`;
        divTask.appendChild(txtTask);
    });
    const nameVect = ["a", "b", "c"]
    data[variant].basis.forEach((el, ind)=> createNameVect(el, nameVect, ind));
    createNameVect(startVector[0], nameVect, 2)
...
btnCheck.addEventListener("click", () => {
    count = 0, mark = 0;
    let coordUserVect = coordVect(arrUserVector);
    data[variant].coordUserVectors = coordUserVect;
    console.log("arrUserVector", arrUserVector, coordUserVect, data[variant].coordTaskVectors)
    let i = 0;
    coordUserVect.forEach((el, ind)=>{
        count += (el.every((val, ind)=>val == data[variant].coordTaskVectors[i][ind])) ? 1 : -1;
        i++;
    });
    mark = count <= 0 ? 0 : Math.round(count * data[variant].score / data[variant].coordTaskVectors.length);
    spanResult.textContent = "Результат: " + mark + " из " + data[variant].score;
    console.log(data, count, coordUserVect, arrUserVector);
})
